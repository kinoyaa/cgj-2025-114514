...
class _PreviewTexture:
	...
	static func _preview(path: String, preview: Texture2D, thumbnail_preview: Texture2D, userdata: Dictionary):
		var callback : Callable = userdata['callback']
...
static func get_res_preview_texture(res_path: String, callback: Callable) -> void:
	...
	previewer.queue_resource_preview(res_path, _PreviewTexture, "_preview", callback_data)
