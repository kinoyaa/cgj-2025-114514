...
static func debug(object: Object, params: Array, sep: String = " "):
	if object.get_script() != null:
		var path : String = (object.get_script() as Script).resource_path 
		print_rich("[color=" + Color.DARK_ORANGE.to_html() + "][ DEBUG ] " + path.get_file() + "[/color]")
		
	print(sep.join(params))
